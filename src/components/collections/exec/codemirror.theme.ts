...
export namespace Theme {
   ...
   * CodeMirror 6 themes
   */
  type ThemeType = any;  // Define this based on what constitutes a theme in your context
  const themeMap: Map<string, ThemeType> = new Map<string, ThemeType>([
    [
      "codemirror",
   ...
   */
  export function defaultTheme(): ThemeType {
   ...
   */
  export function registerTheme(name: string, theme: ThemeType): void {
   ...
   */
  export function getTheme(name: string): ThemeType {
    let ext = themeMap.get(name);
    return ext !== null && ext !== undefined ? ext : defaultTheme();
  }
}
